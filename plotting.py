...
if __name__ == '__main__':
    print('[plotting] started at %s' % (time.strftime('%d-%b-%Y %H:%M')))
    tic = time.clock()
    ...
    axis_left = figure.add_subplot(111, aspect='equal', adjustable='box')
    ...
    for t in range(len(time_series.keys())):
        ...
        agent_positions = time_series[t]['position']
        agent_plans = time_series[t]['plan']
        ...
        if t % settings.meeting_interval == 0:
            for meeting in schedule[next_meetings]:
                ...
                for idx, label in enumerate(meeting):
                    ...
                    x, y = np.asarray(rc_to_xy(settings.dimension, agent_positions[label])) + settings.cell_side_length
        ...
        for label in agent_plotting:
            if agent_in_meeting[label-1] == 1:
                agent_plotting[label]['position'].set_data([], [])
                continue
            x, y = np.asarray(rc_to_xy(settings.dimension, agent_positions[label])) + settings.cell_side_length
            agent_plotting[label]['position'].set_data(x, y)
        ...
        for label in agent_plotting:
            data_x, data_y = [], []
            for element in agent_plans[label]:
                x, y = np.asarray(rc_to_xy(settings.dimension, element)) + settings.cell_side_length
                data_x.append(x)
                data_y.append(y)
            agent_plotting[label]['path'].set_data(data_x, data_y)
            ...
            if data_x and data_y:
                agent_plotting[label]['meetings'].set_data(data_x[-1], data_y[-1])
            elif t == 0:
                x, y = np.asarray(rc_to_xy(settings.dimension, agent_positions[label])) + settings.cell_side_length
                agent_plotting[label]['meetings'].set_data([x], [y])
            else:
                agent_plotting[label]['meetings'].set_data([], [])

        axis_left.figure.canvas.draw()
        filename = 'sim_images/meetings/' + 'iteration' + str(t).zfill(4) + '.png'
    ...
    toc = time.clock()
    dt = toc - tic
    print('[plotting] completed at %s' % (time.strftime('%d-%b-%Y %H:%M')))
    print('[plotting] %0.2fs = %0.2fm = %0.2fh elapsed' % (dt, dt / 60, dt / 3600))
